DROP procedure if exists proc_upsert_sf_dollar_value_fy_q;
create procedure proc_upsert_sf_dollar_value_fy_q(IN dbname varchar(128))
BEGIN
    SET @theSQL=CONCAT('

create or replace view ', dbname,'.sf_dollar_value_fy_q as select sum(`O`.`opp_acv`)             AS `opp_acv`,
