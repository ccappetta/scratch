DROP procedure if exists proc_upsert_sf_dollar_value_fy;
create procedure proc_upsert_sf_dollar_value_fy(IN dbname varchar(128))
BEGIN
    SET @theSQL=CONCAT('

create or replace view ', dbname,'.sf_dollar_value_fy as select sum(`O`.`opp_acv`)             AS `opp_acv`,
